...
def rdivPayGDash(request, pk):
	...
	# 
	tot_cats = f_pay_cat(div, PCategory, None, False)
	tot_secs = f_pay_sec(div, Sector, None, False)
	tot_caps = f_pay_cap(div, Capital, None, False)
...
def rdivPayGYear(request, pk, year):
	...
	if check:
		tot_cont = ContractYear.objects.filter(contract__project__owner=div, contract__is_fiscal=False, year=year).exclude(total=0).aggregate(Sum('total')).get('total__sum', 0.00)
		tot_paid = Payment.objects.filter(contract__project__owner=div, contract__is_fiscal=False, contyear__year=year).exclude(total=0).aggregate(Sum('total')).get('total__sum', 0.00)
		if tot_paid: tot_bal = tot_cont-tot_paid
		# 
		tot_cats = f_pay_cat(div, PCategory, year, False)
		tot_secs = f_pay_sec(div, Sector, year, False)
		tot_caps = f_pay_cap(div, Capital, year, False)
	...
	return render(request, 'reportdiv_pay_g/pay_dash_year.html', context)
### CAT
...
def rdivPayGCatAll(request, pk, pk2, page):
	...
	objects = f_cat_all(div, obj, is_ann=False, is_fiscal=False)
...
def rdivPayGCatYear(request, pk, year, pk2, page):
	...
	div = get_object_or_404(Division, pk=pk)
	obj = get_object_or_404(PCategory, pk=pk2)
	objects = f_cat_year(div, obj, year, is_ann=False, is_fiscal=False)
	subtitle = f'Kategoria {obj.name} ({obj.code})'
	context = {
...
def rdivPayGCatSum(request, pk):
	group = get_roles(request)
	div = get_object_or_404(Division, pk=pk)
	objects = f_cat_sum(div, is_ann=False, is_fiscal=False)	
	...
	obj_b = f_cat_sum_tot(div, objs, is_ann=False, is_fiscal=False)
	totals = [obj_a,obj_b]
...
def rdivPayGCatSumYear(request, pk, year):
	group = get_roles(request)
	div = get_object_or_404(Division, pk=pk)
	objects = f_cat_sum_y(div, year, is_ann=False, is_fiscal=False)
	...
	objs = PCategory.objects.filter().all()
	tot_b = f_cat_sum_tot_y(div, objs, year, is_ann=False, is_fiscal=False)
	totals = [tot_a,tot_b]
...
def rdivPayGCatSumMonth(request, pk, year, month):
	...
	objects = f_cat_sum_m(div, year, month, is_ann=False, is_fiscal=False)
	...
	tot_a = [tot_a_1,tot_a_2,tot_a_3]
	objs = PCategory.objects.filter().all()
	tot_b = f_cat_sum_tot_m(div, objs, year, month, is_ann=False, is_fiscal=False)
	totals = [tot_a,tot_b]
...
def rdivPayGCatSumYearDet(request, pk, year, pk2):
	...
	div = get_object_or_404(Division, pk=pk)
	obj = get_object_or_404(PCategory, pk=pk2)
	objects = f_cat_sum_y_det(div, obj, year, is_ann=False, is_fiscal=False)
...
def rdivPayGCatSumMonthDet(request, pk, year, month, pk2):
	...
	div = get_object_or_404(Division, pk=pk)
	obj = get_object_or_404(PCategory, pk=pk2)
	objects = f_cat_sum_m_det(div, obj, year, month, is_ann=False, is_fiscal=False)
